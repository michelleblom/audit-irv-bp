// Code written by Michelle Blom, 2018
...
void InsertNode(Frontier &front, const Node &node, bool depthfirst){		
	if(!node.expandable){
...
void ReplaceWithBestAncestor(Frontier &front, const Node &newn, 
	...
	InsertNode(front, newnode, false);
...
double PerformDive(const Node &toexpand, const Candidates &cands, 
	...
	for(int i = 0; i < ncands; ++i){
		if(find(toexpand.tail.begin(), toexpand.tail.end(), i) ==
			...
			newn.estimate = FindBestAudit(cands, rep_ballots, logrlimit,
				newn.best_audit, facts, newn.tail, false);
			...
			if(!newn.expandable){
				...
				return PerformDive(newn, cands, rep_ballots, logrlimit, facts); 
			}
			break;
		}
	}
...
int main(int argc, const char * argv[]) 
{
	try{
			...
			for(int j = 0; j < torun.size(); ++j){
				...
				if(spec.wonly){
					...
					Result r = RunAudit(rep_ballots, act_ballots, candidates, 
						rlimit, winners, losers, poll_order);
					if(runlog){
						cout << r.polls << " (" << r.polls/config.totalvotes << 
							") ballot polls required" << endl;
					}
		...
		Frontier front;
		double logrlimit = log(1.0/rlimit);
		...
		for(int i = 0; i < ncands-1; ++i){
			...
			for(int j = i+1; j < ncands; ++j){
				...
				double asn=EstimateSampleSizeWL(rep_ballots,
		...
		for(int i = 0; i < ncands; ++i){
			...
			for(int j = 0; j < ncands; ++j){
				...
				InsertNode(front, newn, depthfirst);
			}
		}
